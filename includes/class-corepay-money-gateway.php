...
class CorePay_Money_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->title          = $this->get_option( 'title', __( 'CorePay Money', 'corepay-gateway-for-woocommerce' ) );
		$this->description    = $this->get_option( 'description', __( 'Pay securely with CorePay Money.', 'corepay-gateway-for-woocommerce' ) );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled'         => array(
				'title'   => __( 'Enable/Disable', 'corepay-gateway-for-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable CorePay Money', 'corepay-gateway-for-woocommerce' ),
				'default' => 'no',
			),
			'title'           => array(
				'title'       => __( 'Title', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'text',
				'description' => __( 'Payment method title shown at checkout.', 'corepay-gateway-for-woocommerce' ),
				'default'     => __( 'CorePay Money', 'corepay-gateway-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'description'     => array(
				'title'       => __( 'Description', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Payment method description shown at checkout.', 'corepay-gateway-for-woocommerce' ),
				'default'     => __( 'Pay securely with CorePay Money.', 'corepay-gateway-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'widget_url'      => array(
				'title'       => __( 'Widget URL', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'url',
				'description' => __( 'CorePay Money hosted widget endpoint.', 'corepay-gateway-for-woocommerce' ),
				'default'     => 'https://corepay.money/widget',
			),
			'operators'       => array(
				'title'       => __( 'Providers', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'operators',
				'description' => __( 'Add one or more providers. Drag rows to control preference order.', 'corepay-gateway-for-woocommerce' ),
			),
			'currency_mode'   => array(
				'title'       => __( 'Currency', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'select',
				'description' => __( 'Use the WooCommerce store currency, or choose a custom CorePay asset while quoting the WooCommerce currency.', 'corepay-gateway-for-woocommerce' ),
				'default'     => 'system',
				'options'     => array(
					'system' => __( 'Use WooCommerce store currency', 'corepay-gateway-for-woocommerce' ),
					'custom' => __( 'Use custom currency', 'corepay-gateway-for-woocommerce' ),
				),
			),
			'custom_currency' => array(
				'title'       => __( 'Custom Currency', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'text',
				'description' => __( 'CorePay asset code, 1 to 6 characters, used only when Currency is set to custom. The WooCommerce store currency is still used as the fiat quote.', 'corepay-gateway-for-woocommerce' ),
				'default'     => '',
			),
			'digitize'        => array(
				'title'       => __( 'Digitize', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'checkbox',
				'label'       => __( 'Keep digitize option enabled for CorePay payloads', 'corepay-gateway-for-woocommerce' ),
				'description' => __( 'Checked by default. The value is sent as digitize: true in custom JSON.', 'corepay-gateway-for-woocommerce' ),
				'default'     => 'yes',
			),
			'signature_key_id' => array(
				'title'       => __( 'Signature Key ID', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'text',
				'description' => __( 'CorePay JWKS key ID used to verify webhook signatures from https://corepay.money/.well-known/jwks.json.', 'corepay-gateway-for-woocommerce' ),
				'default'     => 'corepay-key-1',
			),
			'debug'           => array(
				'title'       => __( 'Debug Log', 'corepay-gateway-for-woocommerce' ),
				'type'        => 'checkbox',
				'label'       => __( 'Enable WooCommerce logger entries', 'corepay-gateway-for-woocommerce' ),
				'default'     => 'no',
			),
		);
	}
	...
	public function admin_options() {
		...
		echo '<h3>' . esc_html__( 'Webhook', 'corepay-gateway-for-woocommerce' ) . '</h3>';
		echo '<p>' . esc_html__( 'Configure CorePay Money to send payment events to this URL:', 'corepay-gateway-for-woocommerce' ) . '</p>';
	...
	public function generate_operators_html() {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label><?php esc_html_e( 'Providers', 'corepay-gateway-for-woocommerce' ); ?></label>
			</th>
			<td class="forminp">
				<table class="widefat corepay-money-operators" data-field-key="<?php echo esc_attr( $field_key ); ?>">
					<thead>
						<tr>
							<th class="corepay-money-operator-handle"><?php esc_html_e( 'Order', 'corepay-gateway-for-woocommerce' ); ?></th>
							<th><?php esc_html_e( 'ID / CORE ID', 'corepay-gateway-for-woocommerce' ); ?></th>
							<th><?php esc_html_e( 'Provider', 'corepay-gateway-for-woocommerce' ); ?></th>
							<th><?php esc_html_e( 'Actions', 'corepay-gateway-for-woocommerce' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php foreach ( $operators as $operator ) : ?>
							<tr>
								...
								<td><input type="text" name="<?php echo esc_attr( $field_key ); ?>[id][]" value="<?php echo esc_attr( $operator['id'] ); ?>" placeholder="<?php esc_attr_e( 'CB…', 'corepay-gateway-for-woocommerce' ); ?>" /></td>
								<td><input type="text" name="<?php echo esc_attr( $field_key ); ?>[operator][]" value="<?php echo esc_attr( $operator['operator'] ); ?>" placeholder="<?php esc_attr_e( 'Provider ID, e.g. ping', 'corepay-gateway-for-woocommerce' ); ?>" /></td>
								<td><button type="button" class="button corepay-money-remove-operator"><?php esc_html_e( 'Delete', 'corepay-gateway-for-woocommerce' ); ?></button></td>
							</tr>
						<?php endforeach; ?>
					</tbody>
				</table>
				<p><button type="button" class="button corepay-money-add-operator"><?php esc_html_e( 'Add provider', 'corepay-gateway-for-woocommerce' ); ?></button></p>
				<p class="description"><?php esc_html_e( 'Add one or more providers. Drag rows to reorder or delete rows you no longer use.', 'corepay-gateway-for-woocommerce' ); ?></p>
			</td>
		</tr>
		<?php
		return ob_get_clean();
	...
	public function process_payment( $order_id ) {
		...
		if ( ! $order ) {
			wc_add_notice( __( 'Unable to create CorePay payment for this order.', 'corepay-gateway-for-woocommerce' ), 'error' );
		...
		$order->add_order_note( __( 'CorePay Money payment payload created. Awaiting hosted widget payment.', 'corepay-gateway-for-woocommerce' ) );
	...
	public function process_scheduled_subscription_payment( $renewal_total, $renewal_order ) {
		...
		if ( 0 >= (float) $renewal_total ) {
			...
			$renewal_order->add_order_note( __( 'CorePay Money completed zero-total subscription renewal.', 'corepay-gateway-for-woocommerce' ) );
			return;
		}

		$payload = $this->store_payment_payload( $renewal_order, 'subscription_renewal' );
		$renewal_order->update_status( 'on-hold', __( 'Awaiting CorePay Money recurring payment webhook confirmation.', 'corepay-gateway-for-woocommerce' ) );
	...
	public function update_failing_payment_method( $original_order, $renewal_order ) {
		...
		$renewal_order->add_order_note( __( 'CorePay Money updated the future recurring payment method after a failed renewal payment.', 'corepay-gateway-for-woocommerce' ) );
	}
	...
	public function receipt_page( $order_id ) {
		...
		if ( ! $order ) {
			echo esc_html__( 'Order not found.', 'corepay-gateway-for-woocommerce' );
			return;
		}
		...
		if ( ! is_array( $operator ) || empty( $operator['id'] ) || empty( $operator['operator'] ) ) {
			echo esc_html__( 'CorePay Money is not configured for this payment.', 'corepay-gateway-for-woocommerce' );
			return;
		}
		...
		<div class="corepay-money-widget-wrap">
			<p><?php esc_html_e( 'Complete your payment in the secure CorePay Money widget.', 'corepay-gateway-for-woocommerce' ); ?></p>
	...
	public function build_payment_payload( $order, $context = 'checkout' ) {
		...
		$order_payload = array(
			...
			'amount'      => wc_format_decimal( $order->get_total(), wc_get_price_decimals() ),
			'currency'    => $currency,
			'description' => sprintf( /* translators: %s: order number */ __( 'WooCommerce order %s', 'corepay-gateway-for-woocommerce' ), $order->get_order_number() ),
		);
